...
def run_analysis(cfg: AnalysisConfig, *, seed: int | None = None, show: bool = False) -> str:
    ...
    # Full run record: configuration header (as '#' comments) + one machine-
    # readable row per trial. This is the single per-trial summary — the old
    # separate SummaryAnalysis_*.txt was redundant with it. Honors the
    # overwrite/append semantics (overwrite=False accumulates trials across
    # runs; each run appends its own '#' header block).
    log_path = f"{summary_dir}/analysis.log"
    log = open(log_path, "w" if cfg.overwrite else "a")
    ...
    log.write("# Trial  Pmax      Betamax(deg)  Relerr    N_obj  AvgPointsPerObj  "
              "N_apparitions_total\n")
    log.flush()
    ...
    for trial in range(cfg.Ntrials):
        ...
        log.write("%5s  %.5f  %10.4f  %.4f  %5i  %3i  %5i\n"
                  % (trial + 1, result.pmax, result.betamax * (180.0 / np.pi),
                     result.relerr, Nobjs, np.median(Npoints_avg),
                     np.sum(Napparlist)))
        log.flush()
    ...
    log.write(f"# completed {cfg.Ntrials} trials at "
              f"{datetime.datetime.now().isoformat(timespec='seconds')}\n")
    log.close()

    # --- summary plots over all trials ---
    # ('#' header/footer lines are stripped by genfromtxt's comment handling)
