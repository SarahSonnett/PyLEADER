...
- **Arguments:**
  ...
  - `--overwrite`
  - `--seed N`
- **Runtime:** ~5–6 min at the defaults (100 trials × 1000 draws); details in
...
- **Photometric noise:** `--noise-model empirical` (default) applies the population's own
  flux–fluxerr relation, calibrated to its measured epoch-to-epoch repeatability, point by
  point (documented as `noise_model.json` + `noise_model_fit.png`); `flat` restores the
  original 1% Gaussian. Full model + calibration rationale:
  [DevelopmentNotes.md](DevelopmentNotes.md).
...
- **Form:** a pair of degree-2 polynomial surfaces in the recovered `(p, β)`, least-squares
  fit to the bias-map points; the LaTeX equations and fitting details are in
  [DevelopmentNotes.md](DevelopmentNotes.md).
...
- **Consistency:** one noise model per basis — `pyleader-basis` warns when resuming a
  directory built under a different one (details: [DevelopmentNotes.md](DevelopmentNotes.md)).
- **Output:**
...
- **Runtime:** the default 12×12 grid × 4 seeds is ~40 min on 8 workers (× 8 seeds doubles
  it; linear in grid points × seeds). Per-unit cost analysis and seed-count guidance
  (precision scaling; resumable seed top-ups): [DevelopmentNotes.md](DevelopmentNotes.md).
...
How Bayes' rule enters, and how to read the 2-D posterior map (including why a diagonal
ridge is the p–β degeneracy made visible, not a numerical problem):
[DevelopmentNotes.md](DevelopmentNotes.md).
...
- **Arguments:**
  ...
  - `--seed N`
- **Runtime:** end-to-end **~50–90 min** for a ~360-object family (the basis dominates;
  everything scales linearly with trials, draws, and basis units). Stage-by-stage
  measurements: [DevelopmentNotes.md](DevelopmentNotes.md).
...
Both the **tabular** (default) and **legacy block** layouts are fully supported —
`read_obs()` auto-detects them, so datasets from prior analyses work unchanged. Column
definitions and both layouts: [DevelopmentNotes.md](DevelopmentNotes.md).
